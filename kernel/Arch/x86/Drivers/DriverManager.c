...
ACPI_STATUS pci_irq_routings_callback(ACPI_RESOURCE *res, void *context)
{
	...
	if (res->Type == ACPI_RESOURCE_TYPE_IRQ) 
	{
		...
		UINT32 offset = ((ACPI_HIWORD(ACPI_LODWORD(tbl->Address))) * 4) + tbl->Pin;
		...
		printf("(Device %u - Pin: %u, Irq: %u) ",
			(uint32_t)ACPI_HIWORD(ACPI_LODWORD(tbl->Address)),
			tbl->Pin, irq->Interrupts[tbl->SourceIndex]);
	}
...
ACPI_STATUS pci_get_features(pci_device_t *device)
{
	...
	status = AcpiGetHandle(device->handle, "_CRS", &null_handle);
