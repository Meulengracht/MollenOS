	...
	switch (GetMachine()->BootInformation.VbeMode) {
		...
		default: {
			if (GetMachine()->BootInformation.VbeModeInfo) {
				...
				Terminal.AvailableOutputs          |= VIDEO_GRAPHICS;
				Terminal.FrameBufferAddress         = vbe->PhysBasePtr;
				Terminal.FrameBufferAddressPhysical = vbe->PhysBasePtr;
				Terminal.Info.Width                 = vbe->XResolution;
				Terminal.Info.Height                = vbe->YResolution;
				Terminal.Info.Depth                 = vbe->BitsPerPixel;
				Terminal.Info.BytesPerScanline      = vbe->BytesPerScanLine;
				Terminal.Info.RedPosition           = vbe->RedMaskPos;
				Terminal.Info.BluePosition          = vbe->BlueMaskPos;
				Terminal.Info.GreenPosition         = vbe->GreenMaskPos;
				Terminal.Info.ReservedPosition      = vbe->ReservedMaskPos;
				Terminal.Info.RedMask               = vbe->RedMaskSize;
				Terminal.Info.BlueMask              = vbe->BlueMaskSize;
				Terminal.Info.GreenMask             = vbe->GreenMaskSize;
				Terminal.Info.ReservedMask          = vbe->ReservedMaskSize;
