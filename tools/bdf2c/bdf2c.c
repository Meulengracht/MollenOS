...
int main(int argc, char *const argv[])
{
    ...
    //
    for (;;) {
    switch (getopt(argc, argv, "bcC:io:n:hO?-")) {
        case 'i':
    {
        if (!(_in = fopen(optarg, "r+"))) {
            fprintf(stderr, "Can't open in-file '%s': %s\n", optarg,
            strerror(errno));
            exit(-1);
        }
        continue;
    ...
    case 'o':
    {
        ...
        if (!(_out = fopen(optarg, "w"))) {
            fprintf(stderr, "Can't open out-file '%s': %s\n", optarg,
            strerror(errno));
            exit(-1);
        }
        continue;
    ...
    {
        if (!(_in = fopen("font.bdf", "r+"))) {
            fprintf(stderr, "Can't open in-file '%s': %s\n", optarg,
            strerror(errno));
            exit(-1);
        }

        if (!(_out = fopen("font.c", "w"))) {
            fprintf(stderr, "Can't open out-file '%s': %s\n", optarg,
            strerror(errno));
            exit(-1);
        }

        ReadBdf(_in, _out, name);
		continue;
    } break;
