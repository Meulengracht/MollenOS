...
static int symbol_demangle(struct parsed_symbol* sym);
 ...
 */
static int str_array_push(struct parsed_symbol* sym, const char* ptr, int len,
                           struct array* a)
{
    char**      new;
...
/* forward declaration */
static int demangle_datatype(struct parsed_symbol* sym, struct datatype_t* ct,
                              struct array* pmt, int in_args);
 ...
 */
static char* get_args(struct parsed_symbol* sym, struct array* pmt_ref, int z_term, 
...

{
    ...
    while (*sym->current)
    {
        ...
        if (!demangle_datatype(sym, &ct, pmt_ref, 1))
            return NULL;
 ...
 */
static int get_modifier(struct parsed_symbol *sym, const char **ret, const char **ptr_modif)
{
    *ptr_modif = NULL;
    if (*sym->current == 'E')
    {
...
static int get_modified_type(struct datatype_t *ct, struct parsed_symbol* sym,
                              struct array *pmt_ref, char modif, int in_args)
{
    const char* modifier;
 ...
 * array.
 */
static int get_class(struct parsed_symbol* sym)
{
    ...
    while (*sym->current != '@')
    {
        ...
        case '?':
            ...
            case '?':
                ...
                break;
            default:
                if (!(name = get_number( sym ))) return 0;
 ...
 * by char 'ch'. Sets export to 1 iff the calling convention is exported.
 */
static int get_calling_convention(char ch, const char** call_conv,
 ...
 */
static int demangle_datatype(struct parsed_symbol* sym, struct datatype_t* ct,
                              struct array* pmt_ref, int in_args)
{
    char                dt;
    int                add_pmt = 1;
 ...
 * a class.
 */
static int handle_data(struct parsed_symbol* sym)
{
 ...
 * a class.
 */
static int handle_method(struct parsed_symbol* sym, int cast_op)
{
    char                accmem;
    const char*         access = NULL;
    int                 access_id = -1;
    const char*         member_type = NULL;
	struct datatype_t   ct_ret = { 0 };
    const char*         call_conv;
    const char*         modifier = NULL;
    const char*         exported;
    const char*         args_str = NULL;
    const char*         name = NULL;
    int                ret = 0, has_args = 1, has_ret = 1;
    unsigned            mark;
    struct array        array_pmt;
    ...
    {
        if (!demangle_datatype(sym, &ct_ret, &array_pmt, 0))
            goto done;
    }
    ...
    mark = sym->stack.num;
    if (has_args && !(args_str = get_args(sym, &array_pmt, 1, '(', ')'))) goto done;
                ...
                case '0':
                    {
                        ...
                        sym->current++;
                        str_array_init(&pmt);
                        demangle_datatype(sym, &ct, &pmt, 0);
