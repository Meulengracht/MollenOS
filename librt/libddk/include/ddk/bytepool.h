...
DDKDECL(void*, bgetr(bytepool_t *pool, void *buf, long size));

/* brel
 * Release a previous allocated buffer in the given pool. */
DDKDECL(void, brel(bytepool_t *pool, void *buf));

/* bectl
 * Establish automatic pool expansion control */
DDKDECL(void, bectl(bytepool_t *pool, 
    int (*compact) (long sizereq, int sequence), 
    void *(*acquire) (long size), void (*release) (void *buf),
    long pool_incr));

/* bstats
 * Return buffer allocation free space statistics.  */
DDKDECL(void, bstats(bytepool_t *pool,long *curalloc, long *totfree, long *maxfree, long *nget, long *nrel));

/* bstatse
 * Return extended statistics */
DDKDECL(void, bstatse(bytepool_t *pool, long *pool_incr, long *npool, 
...
DDKDECL(void, bufdump(bytepool_t *pool, void *buf));
...
DDKDECL(void, bpoold(bytepool_t *pool, void *buf, int dumpalloc, int dumpfree));

/* bpoolv
 * Validate a buffer pool. If NDEBUG isn't defined,
 * any error generates an assertion failure. */
DDKDECL(oserr_t, bpoolv(bytepool_t * pool, void *buf));
