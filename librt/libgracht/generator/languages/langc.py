...
class CGenerator:
        ...
        for param in params:
            ...
            if should_define_param_length_component(param, case):
                parameters_valid.append(
                    self.get_param_typename(protocol, Parameter(param.get_name() + "_length", "size_t"), case))
    ...
    def define_structures(self, protocol, outfile):
        for func in protocol.get_functions():
            if len(func.get_request_params()):
                self.write_structure(protocol, get_input_struct_name(protocol, func), func.get_request_params(),
                                     CONST.TYPENAME_CASE_MEMBER, outfile)
                outfile.write("\n")

        for evt in protocol.get_events():
            if len(evt.get_params()):
                self.write_structure(protocol, get_event_struct_name(protocol, evt), evt.get_params(),
                                     CONST.TYPENAME_CASE_MEMBER, outfile)
                outfile.write("\n")
        return
    ...
    def get_server_callback_prototype(self, protocol, func):
        ...
        function_message_param = self.get_param_typename(protocol, Parameter("message", "struct gracht_recv_message*"),
    ...
    def get_function_prototype(self, protocol, func, case):
        ...
        function_context_param = self.get_param_typename(protocol,
    ...
    def get_size_function(self, protocol, param, case):
        ...
        if param.get_count() != "1":
            return "sizeof(" + self.get_param_typename(protocol, param,
    ...
    def define_status_struct(self, protocol, params_out, case, outfile):
    ...
    def define_function_body(self, protocol, func, outfile):
        ...
        self.define_message_struct(protocol, func.get_id(), func.get_request_params(), func.get_response_params(),
    ...
    def define_status_body(self, protocol, func, outfile):
        self.define_status_struct(protocol, func.get_response_params(),
    ...
    def define_event_body_single(self, protocol, evt, outfile):
        self.define_message_struct(protocol, evt.get_id(), evt.get_params(), [], "MESSAGE_FLAG_EVENT",
    ...
    def define_response_body(self, protocol, func, flags, outfile):
        self.define_message_struct(protocol, func.get_id(), func.get_response_params(), [], flags,
    ...
    def define_events(self, protocol, outfile):
        for evt in protocol.get_events():
            outfile.write(
                self.get_protocol_event_prototype_name_single(protocol, evt, CONST.TYPENAME_CASE_FUNCTION_CALL) + "\n")
            outfile.write("{\n")
            self.define_event_body_single(protocol, evt, outfile)
            outfile.write("}\n\n")

            outfile.write(
                self.get_protocol_event_prototype_name_all(protocol, evt, CONST.TYPENAME_CASE_FUNCTION_CALL) + "\n")
            outfile.write("{\n")
    ...
    def write_protocol_event_prototype(self, protocol, evt, outfile):
        outfile.write("    " + self.get_protocol_event_prototype_name_single(protocol, evt,
                                                                             CONST.TYPENAME_CASE_FUNCTION_CALL) + ";\n")
        outfile.write("    " + self.get_protocol_event_prototype_name_all(protocol, evt,
                                                                          CONST.TYPENAME_CASE_FUNCTION_CALL) + ";\n")
        return

    def get_client_protocol_event_callback(self, protocol, evt):
        prototype = "void " + self.get_protocol_client_event_callback_name(protocol, evt) + "("
        if len(evt.get_params()) > 0:
        ...
        if len(protocol.get_functions()) > 0:
            ...
            for func in protocol.get_functions():
                ...
                outfile.write(" *    { " + func_definition + ", "
                              + self.get_protocol_server_callback_name(protocol, func) + " },\n")
            outfile.write(" * };\n")
            outfile.write(" */\n\n")
