...
PACKED_TYPESTRUCT(PeOptionalHeader, {
    // Architecture is the unsigned integer that identifies the state ofthe image file.
    // The most common number is0x10B, which identifies it as a normal executablefile.
    // 0x107 identifies it as a ROM image, and
...
PACKED_TYPESTRUCT(PeOptionalHeader32, {
    ...
    // BaseOfData is the address that is relative to the image base of
    // the beginning-of-data section when it is loaded into memory.
    uint32_t BaseOfData;
    // BaseAddress is the preferred address of the first byte of image when loaded into memory;
    ...
    // SectionAlignment is the alignment (in bytes) of sections when they are loaded into memory.
    // It must be greater than or equal to FileAlignment.
    // The default is the pagesize for the architecture.
...
PACKED_TYPESTRUCT(PeSectionHeader, {
    // Name is an 8-byte, null-padded UTF-8 encoded string. If the string is exactly 8 characters long,
    ...
    // names longer than 8 characters. Long names in object files are truncated if they are
    ...
    // VirtualAddress is for executable images, the address of the first byte of the section
    ...
    // for simplicity, compilers should set this to zero. Otherwise, it is an arbitrary value that
    // is subtracted from offsets during relocation.
    uint32_t VirtualAddress;
    // RawSize is the size of the section (for object files) or the size of the initialized data
    // on disk (for image files). For executable images, this must be a multiple of FileAlignment
    // from the optional header. If this is less than VirtualSize, the remainder of the section is
    // zero-filled. Because the SizeOfRawData field is rounded but the VirtualSize field is not, it
    // is possible for SizeOfRawData to be greater than VirtualSize aswell. When a section contains
    // only uninitialized data, this field should be zero.
    uint32_t RawSize;
    // RawAddress is the file pointer to the first page of the section within the COFF file.
    // For executable images, this must be a multiple of FileAlignment from the optional header.
    // For object files, the value should be aligned on a 4-byte boundary for best performance.
    // When a section contains only uninitialized data, this field should be zero.
    uint32_t RawAddress;
    // PointerToFileRelocations is the file pointer to the beginning of relocation entries
    // for the section. This is set to zero for executable images or if there are no relocations.
    uint32_t PointerToFileRelocations;
    // PointerToFileLineNumbers is the file pointer to the beginning of line-number
    // entries for the section. This is set to zero if there are no COFF line numbers.
    // This value should be zero for an image because COFF debugging information is deprecated.
    uint32_t PointerToFileLineNumbers;
    // NumRelocations is the number of relocation entries for the section. This is
    // set to zero for executable images.
    uint16_t NumRelocations;
    // NumLineNumbers is the number of line-number entries for the section. This value
