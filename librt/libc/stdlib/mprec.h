...
typedef union { double d; unsigned long i[2]; } U;
...
_Bigint *s2b(const char*, int, int, unsigned long);
_Bigint	*i2b(int);
_Bigint *mult(_Bigint *, _Bigint *);
_Bigint *pow5mult(_Bigint *, int k);
int hi0bits(unsigned long);
int lo0bits(unsigned long*);
...
unsigned long any_on(_Bigint *b, int k);
void copybits(unsigned long *c, int n, _Bigint *b);
...
int		_strtodg_l (const char *s00, char **se, struct FPI *fpi, 
	Long *exp, __ULong *bits, locale_t);
...
#endif /* !defined(PREFER_SIZE_OVER_SPEED) && !defined(__OPTIMIZE_SIZE__) && !defined(_SMALL_HEXDIG) */
#ifdef INFNAN_CHECK
int	hexnan(const char **sp, const struct FPI *fpi, unsigned long *x0);
#endif
