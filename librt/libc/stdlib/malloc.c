...
#else /* LACKS_UNISTD_H */
#if !defined(__FreeBSD__) && !defined(__OpenBSD__) && !defined(__NetBSD__) && !defined(__VALI__)
...
static FORCEINLINE void* mosmmap(size_t Size) {
	...
	if (MemoryAllocate(NULL, Size, 
	    MEMORY_COMMIT | MEMORY_CLEAN | MEMORY_READ | MEMORY_WRITE, &ptr) != OS_EOK) {
		return MFAIL;
	}
	else {
		return ptr;
...
static FORCEINLINE int mosmunmap(void* Ptr, size_t Size) {
	return MemoryFree(Ptr, Size) == OS_EOK ? 0 : -1;
}
