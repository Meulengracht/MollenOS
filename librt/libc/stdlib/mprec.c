...
_Bigint *Balloc(int k)
{
	...
	_Bigint *rv ;
	x = 1 << k;
	rv = (_Bigint *)calloc(1, sizeof(_Bigint) +
	  (x - 1) * sizeof(rv->_x));
	if (rv == NULL) 
		return NULL;
	rv->_k = k;
...
_Bigint *s2b(const char *s, int nd0, int nd, __ULong y9)
{
  _Bigint *b;
  int i, k;
  __Long x, y;
...
_Bigint *mult(_Bigint * a, _Bigint * b)
{
  _Bigint *c;
  int k, wa, wb, wc;
  __ULong carry, y, z;
...
_Bigint *d2b(double _d, int *e, int *bits)
