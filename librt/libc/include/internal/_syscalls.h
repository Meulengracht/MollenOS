...
#define Syscall_WaitQueueCreate(HandleOut)                                 (OsStatus_t)syscall1(47, SCPARAM(HandleOut))
#define Syscall_WaitQueueBlock(Handle, SyncObject, Timeout)                (OsStatus_t)syscall3(48, SCPARAM(Handle), SCPARAM(SyncObject), SCPARAM(Timeout))
#define Syscall_WaitQueueUnblock(Handle)                                   (OsStatus_t)syscall1(49, SCPARAM(Handle))

#define Syscall_CreatePipe(Flags, HandleOut)                               (OsStatus_t)syscall2(50, SCPARAM(Flags), SCPARAM(HandleOut))
#define Syscall_DestroyPipe(Handle)                                        (OsStatus_t)syscall1(51, SCPARAM(Handle))
#define Syscall_ReadPipe(Handle, Buffer, Length)                           (OsStatus_t)syscall3(52, SCPARAM(Handle), SCPARAM(Buffer), SCPARAM(Length))
#define Syscall_WritePipe(Handle, Buffer, Length)                          (OsStatus_t)syscall3(53, SCPARAM(Handle), SCPARAM(Buffer), SCPARAM(Length))

#define Syscall_RemoteCall(RemoteCall, Asynchronous)                       (OsStatus_t)syscall2(54, SCPARAM(RemoteCall), SCPARAM(Asynchronous))
#define Syscall_RpcGetResponse(RemoteCall)                                 (OsStatus_t)syscall1(55, SCPARAM(RemoteCall))
#define Syscall_RemoteCallWait(Handle, RemoteCall, ArgumentBuffer)         (OsStatus_t)syscall3(56, SCPARAM(Handle), SCPARAM(RemoteCall), SCPARAM(ArgumentBuffer))
#define Syscall_RemoteCallRespond(RemoteAddress, Buffer, Length)           (OsStatus_t)syscall3(57, SCPARAM(RemoteAddress), SCPARAM(Buffer), SCPARAM(Length))

#define Syscall_MemoryAllocate(Size, Flags, Virtual, Physical)             (OsStatus_t)syscall4(58, SCPARAM(Size), SCPARAM(Flags), SCPARAM(Virtual), SCPARAM(Physical))
#define Syscall_MemoryFree(Pointer, Size)                                  (OsStatus_t)syscall2(59, SCPARAM(Pointer), SCPARAM(Size))
#define Syscall_MemoryProtect(MemoryPointer, Length, Flags, PreviousFlags) (OsStatus_t)syscall4(60, SCPARAM(MemoryPointer), SCPARAM(Length), SCPARAM(Flags), SCPARAM(PreviousFlags))
#define Syscall_CreateBuffer(Size, DmaBufferPointer)                       (OsStatus_t)syscall2(61, SCPARAM(Size), SCPARAM(DmaBufferPointer))
#define Syscall_AcquireBuffer(Handle, DmaBufferPointer)                    (OsStatus_t)syscall2(62, SCPARAM(Handle), SCPARAM(DmaBufferPointer))
#define Syscall_QueryBuffer(Handle, DmaOut, CapacityOut)                   (OsStatus_t)syscall3(63, SCPARAM(Handle), SCPARAM(DmaOut), SCPARAM(CapacityOut))

#define Syscall_DestroyHandle(Handle)                                      (OsStatus_t)syscall1(64, SCPARAM(Handle))
